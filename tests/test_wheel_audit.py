...
DIST_INFO = "color_palette_skill-0.13.0.dist-info"
...
def _wheel(
    ...
    *,
    ...
    filename: str = "color_palette_skill-0.13.0-py3-none-any.whl",
) -> Path:
    files = {
        "color_palette/__init__.py": b'__version__ = "0.13.0"\n',
...
def test_valid_pure_python_wheel_passes(tmp_path):
    ...
    result = audit_wheel(path, expected_version="0.13.0")
...
def test_cli_accepts_directory_for_windows_compatible_glob_handling(tmp_path):
    _wheel(tmp_path)
    assert main([str(tmp_path), "--expected-version", "0.13.0"]) == 0
...
def test_ci_audits_wheel_before_install_and_smoke():
    ...
    assert "--expected-version 0.13.0" in workflow
